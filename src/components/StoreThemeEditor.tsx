...
export function StoreThemeEditor({ theme, storeName, onSave }: StoreThemeEditorProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid lg:grid-cols-2 gap-8">
      <div className="space-y-4">
        <h3 className="font-display font-bold text-coffee">Colores y estilo</h3>
        ...
        <ColorField
          ...
          onChange={(v) => setForm((f) => ({ ...f, themeAccentColor: v }))}
        />
        <ColorField
          label="Fondo de la tienda"
          value={form.themeBackgroundColor}
          onChange={(v) => setForm((f) => ({ ...f, themeBackgroundColor: v }))}
        />
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, themeButtonStyle: e.target.value as ButtonStyle }))
            }
        ...
        <div>
          ...
          <input
            value={form.themeHeroTitle}
            onChange={(e) => setForm((f) => ({ ...f, themeHeroTitle: e.target.value }))}
            className="w-full px-4 py-3 rounded-xl border border-cream bg-white"
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            onChange={(e) =>
              setForm((f) => ({
                ...f,
                themeHeroSubtitle: e.target.value,
        ...
        <button
          type="submit"
          disabled={saving}
          className="px-6 py-3 bg-coffee text-white rounded-full font-semibold disabled:opacity-50"
        >
      ...
      <div>
        ...
        <div
          className="rounded-2xl border border-cream overflow-hidden"
          style={{ backgroundColor: form.themeBackgroundColor }}
        >
          <div
            className="p-8 text-white"
            style={{
              background: `linear-gradient(135deg, ${form.themePrimaryColor}, ${form.themeAccentColor})`,
            }}
          >
          ...
          <div className="p-6">
            <button
              ...
              className={`px-6 py-2 text-white text-sm font-semibold ${
                form.themeButtonStyle === "pill"
                  ? "rounded-full"
                  : form.themeButtonStyle === "rounded"
                    ? "rounded-xl"
                    : "rounded-lg"
              }`}
            >
    ...
    <div>
      ...
      <div className="flex gap-3 items-center">
        <input
          ...
          className="w-12 h-12 rounded-lg border border-cream cursor-pointer"
        />
        <input
          type="text"
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className="flex-1 px-4 py-3 rounded-xl border border-cream bg-white font-mono text-sm"
        />
      </div>
    </div>
  );
